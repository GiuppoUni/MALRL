...
class multiAGEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.vehicle_names = [v for v in utils.g_airsim_settings["Vehicles"] ]    
        self.episodeN = 0
        self.stepN = 0 
    ...
    def step(self, action,agent_name):
        ...
        # ACTION EXECUTION #
        #=================#
        collided = client.take_action(action,agent_name)
        #=================#
        # #
        #=================#

        now = client.getPosition()
        track = client.goal_direction(self.goal, now) 
        ...
        self.state = client.getScreenDepthVis(track)
