"""
Script to randomly (no RL) generate grid pattern UAV fligth trajectories then hor and ver separated
"""
...
for j in range(0,NUM_TESTS):
   ...
   zeroStart=time.time()
   fids=[]
   for i in range(NUM_TRAJECTORIES[j]):
      # with open(os.devnull, "w") as f, contextlib.redirect_stdout(f):
      start = time.time()
      trajs2d = trajs_utils.random_gen_2d(0,860,0,860,step=120,n_trajs=BUFFER_SIZE)
      end = time.time()
      ...
      with open(os.devnull, "w") as f, contextlib.redirect_stdout(f):
         start = time.time()
      res, i_outs,lfids =  trajs_utils.vertical_separate(trajs2d,fids= [ n+i*BUFFER_SIZE for n in range(0,BUFFER_SIZE) ],
