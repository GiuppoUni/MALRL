...
class AirSimClientBase:
    ...
    def getHomeGeoPoint(self, vehicle_name = STD_NAME):
        return GeoPoint.from_msgpack(self.client.call('getHomeGeoPoint',vehicle_name))

    # basic flight control
    def enableApiControl(self, is_enabled,vehicle_name = STD_NAME):
        return self.client.call('enableApiControl', is_enabled,vehicle_name)
    def isApiControlEnabled(self, vehicle_name = STD_NAME):
    ...
    # simGetImage returns compressed png in array of bytes
    # image_type uses one of the AirSimImageType members
    def simGetImages(self, requests,vehicle_name = STD_NAME):
        responses_raw = self.client.call('simGetImages', requests,vehicle_name)
        return [ImageResponse.from_msgpack(response_raw) for response_raw in responses_raw]

    def simGetCollisionInfo(self,vehicle_name = STD_NAME):
...
class MultirotorClient(AirSimClientBase, object):
    ...
    def getPosition(self,vehicle_name = ""):
        bPos = self.client.call("getMultirotorState",vehicle_name)[b"kinematics_estimated"][b"position"]
        return decodeDict(bPos)
    
    def getOrientation(self,vehicle_name = ""):
        bPos = self.client.call("getMultirotorState",vehicle_name)[b"kinematics_estimated"][b"orientation"]
        return decodeDict(bPos)
    ...
    def getMultirotorState(self,vehicle_name):
        return self.client.call("getMultirotorState",vehicle_name)
    ...
    def moveByVelocity(self, vx, vy, vz, duration, drivetrain = DrivetrainType.MaxDegreeOfFreedom, yaw_mode = YawMode(),vehicle_name = STD_NAME):
        return self.client.call('moveByVelocity', vx, vy, vz, duration, drivetrain, yaw_mode,vehicle_name)

    def moveByVelocityZ(self, vx, vy, z, duration, drivetrain = DrivetrainType.MaxDegreeOfFreedom, yaw_mode = YawMode(),vehicle_name = STD_NAME):
        return self.client.call('moveByVelocityZ', vx, vy, z, duration, drivetrain, yaw_mode, vehicle_name)

    def moveOnPath(self, path, velocity, max_wait_seconds = 60, drivetrain = DrivetrainType.MaxDegreeOfFreedom, yaw_mode = YawMode(), lookahead = -1, adaptive_lookahead = 1, vehicle_name = STD_NAME):
        return self.client.call('moveOnPath', path, velocity, max_wait_seconds, drivetrain, yaw_mode, lookahead, adaptive_lookahead,vehicle_name)
    ...
    def moveToZAsync(self, z, velocity, timeout_sec = 3e+38, yaw_mode = YawMode(), lookahead = -1, adaptive_lookahead = 1, vehicle_name = ''):
        return self.client.call('moveToZ', z, velocity, timeout_sec, yaw_mode, lookahead, adaptive_lookahead,vehicle_name )
    ...
    lookahead = -1, adaptive_lookahead = 1, vehicle_name = ''):
        return self.client.call("moveToPosition", x, y, z, velocity, timeout_sec, drivetrain, yaw_mode, lookahead, adaptive_lookahead, vehicle_name)
    ...
    def moveByManualAsync(self, vx_max, vy_max, z_min, duration, drivetrain = DrivetrainType.MaxDegreeOfFreedom, yaw_mode = YawMode(), vehicle_name = ''):
        return self.client.call('moveByManualAsync', vx_max, vy_max, z_min, duration, drivetrain, yaw_mode, vehicle_name)

    def rotateToYaw(self, yaw, max_wait_seconds = 60, margin = 5,vehicle_name = STD_NAME):
        return self.client.call('rotateToYaw', yaw, max_wait_seconds, margin,vehicle_name)

    def rotateByYawRate(self, yaw_rate, duration,vehicle_name = STD_NAME):
        return self.client.call('rotateByYawRate', yaw_rate, duration,vehicle_name )
...
def decodeDict(bDict):
    """
    From {b"x_vel":1.0,...} to {"x_vel":1.0,...}  
    """

    return { str(bk,'utf-8'):( str(bv,'utf-8') if type(bv) == bytes else bv)  \
